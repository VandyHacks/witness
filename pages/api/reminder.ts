...
export default async function handler(req: NextApiRequest, res: NextApiResponse<any>) {
	...
	switch (req.method) {
        case 'GET': 
            const logins = await User.find({applicationStatus: ApplicationStatus.CREATED}).select('id name email userType applicationStatus');
            // read usertype from vaken db
            return res.status(200).send(logins);
		case 'POST':
            const users = await User.find({applicationStatus: ApplicationStatus.CREATED}).select('id name email userType applicationStatus');
